...
// NewGameTimeMyEventsView
// -----------------------
// "New Game Time" → Events › My Events list. The Discover view with the
// fetcher swapped to GET /v2/discover/events/mine (auth required) — events
// the current user owns / manages. Optimised replacement for the legacy
// customerfrontend NewGameTime/Events/MyEvents.vue
// (`event/calenderEventListss` + `game/gameListing`).
//
// SCOPE NOTE: the v2 `mine` endpoint scopes to events the user CREATED
// (team_events.created_by) plus events under associations the user
// administers (role 1/2) — see the controller docblock. The broad
// Firestore-team-membership union of the legacy tab is intentionally not
// reproduced (it cannot be done without N+1 / Firestore coupling).
//
...
// No invented UI. The Follow toggle stays (reused unchanged).
//
// Data: src/api/discoverEvents.ts → fetchMyEvents.
...
// Association + State + Type filters (selected display names → ids on the wire).
...
async function loadFilterCatalogues() {
  const [assocPage, types] = await Promise.all([
    fetchDiscoverAssociations({ perPage: 50 }),
    fetchEventTypes()
  ])
  associationOptions.value = assocPage.associations.map((a) => a.name)
  assocNameToId = new Map(assocPage.associations.map((a) => [a.name, a.id]))
...
watch([yearFilter, pastEventsFilter, associationFilter, stateFilter, typeFilter], () => {
...
onMounted(() => {
  ...
  void fetchEventYears('mine').then((y) => { if (y.length) availableYears.value = y })
  ...
  <section class="association-users__main">
    ...
    >
      ...
      <div v-else-if="events.length === 0" class="association-users__empty">
        ...
        <p v-else>No events found. We couldn't find any events you own or manage.</p>
      </div>
