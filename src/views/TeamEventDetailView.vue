...
<style scoped>
.event-detail{--ed-bg:var(--body-bg);--ed-border:var(--border-divider);--ed-text:var(--text);--ed-muted:var(--secondary);--ed-blue:var(--primary);color:var(--ed-text);background:var(--ed-bg);width:100%;margin:0;padding:0;min-height:calc(100vh - 64px);letter-spacing:0}
...
/* Preserved: attendee / member / game ⋯ dropdown menus (dark idiom). */
.ed-person__menu,.ed-game__menu{position:relative}
...
/* Preserved: roster group tabs inside the RSVP card. */
.ed-roster-tabs{display:flex;gap:20px;border-bottom:1px solid var(--ed-border);margin-top:16px}
.ed-roster-tabs button{border:0;border-bottom:3px solid transparent;background:none;color:var(--ed-muted);padding:10px 0;cursor:pointer}
.ed-roster-tabs button.is-active{color:var(--primary);border-color:var(--ed-blue)}
.ed-roster-tabs b{background:var(--surface-raised);border-radius:10px;padding:2px 6px}
/* Preserved: tab loading + error/empty states. */
.ed-empty,.ed-error{display:grid;place-items:center;text-align:center;min-height:300px;color:var(--ed-muted)}
.ed-empty h2,.ed-error h2{color:var(--ed-text);margin:8px 0 0}
.ed-empty p,.ed-error p{margin:6px 0 14px}
.ed-empty button,.ed-error button{background:var(--ed-blue);border:0;color:#fff;padding:9px 18px;border-radius:7px}
.ed-tab-loading{display:grid;gap:10px}
.ed-shimmer{display:block;background:linear-gradient(90deg,var(--shimmer-start),var(--shimmer-mid),var(--shimmer-end));background-size:220% 100%;animation:participation-shimmer 1.5s linear infinite}
.ed-shimmer--row{height:120px;border-radius:8px}
</style>
