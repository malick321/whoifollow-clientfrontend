...
// NewGameTimeDiscoverAssociationsView
// -----------------------------------
// Public "New Game Time" → Associations › Discover list. Optimised
// replacement for the legacy customerfrontend
// NewGameTime/Associations/DiscoverAssociations.vue.
//
// Mirrors the NewGameTimeDiscoverEventsView slice EXACTLY (sticky header +
// continuous-scroll list + a Follow toggle) and REUSES his global classes
// (.association-users__* / .association-followers__* / .association-events__*)
// + shared components (AppIcon, TeamAvatar) so it drops into his design
// system unchanged. No invented UI.
//
// Data: src/api/discoverAssociations.ts → GET /v2/discover/associations
// (auth-aware). Contract: docs/api/newgametime-associations-api-contract.md.

import { computed, onBeforeUnmount, onMounted, ref, watch } from 'vue'
import AppIcon from '../components/AppIcon.vue'
import NgtAssociationCard from '../components/gametime/NgtAssociationCard.vue'
import {
  fetchDiscoverAssociations,
  followDiscoverAssociation,
...
async function loadPage(mode: 'reset' | 'append') {
  ...
  try {
    const result = await fetchDiscoverAssociations({
      search: search.value,
      page: nextPage,
      perPage: PAGE_SIZE
    })
...
async function toggleFollow(assoc: DiscoverAssociation) {
  if (followBusy.value.has(assoc.id)) return
  followBusy.value = new Set(followBusy.value).add(assoc.id)
  const wasFollowing = assoc.isFollowing
  try {
    if (wasFollowing) {
      await unfollowDiscoverAssociation(assoc.id)
      assoc.isFollowing = false
      assoc.followId = undefined
    } else {
      const followId = await followDiscoverAssociation(assoc.id)
      assoc.isFollowing = true
      assoc.followId = followId
    }
  } catch (error) {
    pushToast({
      tone: 'warning',
...
<template>
  ...
  <section class="association-users__main">
    ...
    <div v-else-if="associations.length === 0" class="association-users__empty">
      <p v-if="search.trim()">No associations match "{{ search }}".</p>
      <p v-else>No associations found. Try broadening your search.</p>
    </div>
